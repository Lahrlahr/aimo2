...
class BaseGateway:
    def __init__(
        self, data_paths: Tuple[str] = None, file_share_dir: str = _FILE_SHARE_DIR
    ):
    ...
    def validate_prediction_batch(
        ...
        if row_id_colname not in prediction_batch.columns:
            raise GatewayRuntimeError(
                GatewayRuntimeErrorType.INVALID_SUBMISSION,
                f"Prediction missing column {row_id_colname}",
            )
        if not pl.Series(prediction_batch[row_id_colname]).equals(
            pl.Series(sample_submission_batch[row_id_colname])
        ):
            raise GatewayRuntimeError(
                GatewayRuntimeErrorType.INVALID_SUBMISSION,
    ...
    def _standardize_and_validate_paths(
        ...
        if not os.path.exists(self.file_share_dir) or not os.path.isdir(
            self.file_share_dir
        ):
